"use strict";
  ...
  var margins = {'top' : 100, 'right':60, 'left':60, 'bottom':50 };
  var svgWidth = 1000;
  var svgHeight = 510;
  ...
  var colorMapping  = {'XS': '#441005','S':'#b2182b', 'M':'#d6604d',
  ...
  var initialState = {'Gender': {'data':  genderOpts[0],
                        ...
                        'Fit': "None",
                        'Method':['Chest'],
                        'Scatter': {'x': 'Chest', 'y':'Waist'},
                        'Units':'false',
                        'explanation':'false',
                        'table':'false'};

  //Initialize the variable to keep track of user selections
  var stateOptions = Object.assign({}, initialState);


  /*add elements to the graphic*/
  //insert div as container for everything
  ...
  var introBlurb = spMainBlock.append('p')
    .attr('class','intro').style('width','520px');

  introBlurb.append('p')
    .text('A visual summary of clothing brand sizing charts based on ')
    .attr('class','intro_t')
    .style('display', 'inline');

  var dataSource = introBlurb.append('p')
    .text('1,774 men and 2,208 women.')
  ...
  dataSource.append('p')
    .text('Data described in: Gordon, C. C. et al. 1988 ANTHROPOMETRIC' +
          ' SURVEY OF U.S. ARMY PERSONNEL: ' +
          'METHODS AND SUMMARY STATISTICS. (1989).')
    .classed('data-dropdown',true);
...
/*         Add Menu and menu options      */
  ...
  var dataGenderSelector = makeDropdown(select, genderOpts, 'Data Gender',
  ...
  var brandSelector = makeDropdown(select, brandList, 'Brand Chart',
                                   'Brand', 'Select', 115);
  //add on click function to the dropdown options
  ...
  histograms.append('text')
    ...
    .attr('dy','-2em')
    .text('People Distributions')
    .attr('text-anchor','middle')
    .attr('class', 'labels');

  //call functions to build the graphic
  updateGraphs();
  //creates the initial scatter plot and the histograms
...
'Anthropometric measurements are taken form the 1988 Army survey, described in the source: "Gordon, C. C. et al. 1988 ANTHROPOMETRIC SURVEY OF U.S. ARMY PERSONNEL: METHODS AND SUMMARY STATISTICS. (1989)." Although the full dataset is available online, the provider requested citation be directed to that descriptive reference.',
  ...
  */
  ...
  function updateMethodButton(){
    ...
    var methodListFiltered = methodList.filter(function(items) {
    //filter methods to return only ones that correspond to the current size chart
      return items.order.every( function(item){
        return measuredDims.includes(item);
        });
       })
...
function makeScatter(data, svgSelection, xy_spec_obj) {
 ...
 function updateScatter( xy_spec_obj ) {
  ...
  function facetBySize(data) {
    ...
    var sizeFacets = {'XS':[],'S':[],'M':[],'L':[],'XL':[],'XXL':[], 'None':[]};
